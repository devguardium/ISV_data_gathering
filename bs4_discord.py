...
all_files = glob.glob(
    unquote(
        "/home/bt/dev/Med%C5%BAuslovjanska%20bes%C4%9Bda.%20Arhiv%20od%2023-08/*.html"
    )
)
...
for fname in all_files:
    ...
    for entry in tqdm(soup.find_all("div", {"class": "chatlog__message-group"})):
        ...
        if koristniki[aut_nick].get("uid", uid) != uid:
            raise NameError(
                f"duplicate! '{koristniki[aut_nick]['uid']}' and '{uid}' for '{aut_nick}'"
            )
        ...
        for msg in entry.find_all("div", {"class": "chatlog__message"}):
            ...
            koristniki[aut_nick]["total_len"] = koristniki[aut_nick].get(
                "total_len", 0
            ) + len(msg.text)
            koristniki[aut_nick]["num_messages"] = (
                koristniki[aut_nick].get("num_messages", 0) + 1
            )
