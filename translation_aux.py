...
def infer_pos(details_string):
    arr = [
        x
        for x in details_string.replace("./", "/").replace(" ", "").split(".")
        if x != ""
    ]
...
def prepare_slovnik(slovnik):
    for lang in LANGS:
        assert slovnik[
            slovnik[lang].astype(str).apply(lambda x: "((" in sorted(x))
        ].empty
    import re
    ...
    slovnik["isv"] = (
        slovnik["isv"].str.replace("!", "").str.replace("#", "").str.lower()
    )
...
def inflect_carefully(morph, isv_lemma, inflect_data, verbose=False):
    ...
    candidates = {
        form[1]: form.tag.grammemes & inflect_data
        for form in lexeme
    ...
    best_candidates = {
        k: v for k, v in candidates.items() if len(v) == len(best_fit[1])
    }
    if len(best_fit[1]) == 0 and len(inflect_data) > 0:
        if verbose:
            print(
                "have trouble in finding anything like ",
                inflect_data,
                " for ",
                isv_lemma,
            )
