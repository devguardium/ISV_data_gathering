...
def translate_sentence(sent, src_lang, slovnik, etm_morph):
    ...
    for idx, token_row_data in sent.iterrows():
        ...
        if not found:
            shallow_translated = reverse_flavorize(
                token_row_data.form, token_row_data.pos, token_row_data.feats, src_lang
            )
    ...
    for idx, token_row_data in translation_details.iterrows():
        ...
        if original_word.upper() == original_word:
            ...
            translation_candidates = [
                x[0].upper() + x[1:] for x in translation_candidates
            ]
        ...
        if token_row_data.pos == "PUNCT":
            ...
            result_array.append(
                {
                    "str": select_by_naive_levenshtein(
                        translation_candidates, original_word
                    ),
        ...
        if token_row_data.misc:
            space_after = token_row_data.misc.get(
                "SpaceAfter"
            ) or token_row_data.misc.get("SpacesAfter")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--text",
        type=str,
        default="Этот текст стоит тут для примера.",
