...
from isv_nlp_utils.normalizacija import (
    normalize_and_simple_spellcheck,
    transliterate_cyr2lat,
    fix_text,
)

# from example2 import print_spellcheck
from isv_nlp_utils.constants import (
    DISCORD_USERNAME_REGEX,
    create_analyzers_for_every_alphabet,
)
...
for fname in all_files[:]:
    ...
    for entry in soup.find_all("div", {"class": "chatlog__message-group"}):
        ...
        for msg in entry.find_all("div", {"class": "chatlog__content"}):
            ...
            # print("------")
            i += 1
            best_orthography, changed_text, mean_score = (
                normalize_and_simple_spellcheck(msg.text, abecedas)
            )
...
all_unknowns = [
    transliterate_cyr2lat(x)
    for x in all_unknowns
    if len(x) > 1
    and x
    not in {
        "the",
